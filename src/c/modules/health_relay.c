...
//
// modules/health_relay — V32 Static Msg Fix
//
...
static void send_health_snapshot(void) {
  ...
  int32_t steps_to_send = (steps_today > 0) ? steps_today : steps_24h;
	...
	APP_LOG(APP_LOG_LEVEL_INFO, "RELAY V32: Sent Steps:%ld", (long)steps_to_send);
}
...
static void tick_handler(struct tm *tick_time, TimeUnits units_changed) {
  if (units_changed & MINUTE_UNIT) {
    if (tick_time->tm_min % 5 == 0) {
      APP_LOG(APP_LOG_LEVEL_INFO, "RELAY V32: 5-min tick");
      send_health_snapshot();
    }
  }
}
...
static void startup_timer_handler(void *context) {
  ...
  APP_LOG(APP_LOG_LEVEL_INFO, "RELAY V32: Initial push");
	send_health_snapshot();
}
...
void health_relay_init(void) {
  APP_LOG(APP_LOG_LEVEL_INFO, "=== BUILD MARKER: V32_STATIC_MSG_FIX ===");
#ifdef PBL_HEALTH
